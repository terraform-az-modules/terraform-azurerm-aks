...
variable "kv_logs" {
  description = "The kv_logs variable is designed to configure logging settings for Azure Kubernetes Service (AKS) deployments, facilitating the management and monitoring of logs generated by the Kubernetes cluster. This variable is defined as an object, allowing for structured logging configurations that can be tailored to specific requirements.

Attributes:
- log_analytics_workspace_id: A string representing the ID of the Azure Log Analytics workspace where logs will be sent. This is essential for centralized log management and analysis.
- retention_days: An integer specifying the number of days to retain logs in the Log Analytics workspace. This helps manage storage costs and compliance with data retention policies.
- enabled: A boolean flag indicating whether logging is enabled. Setting this to false will disable logging for the AKS cluster.

Notes:
- Ensure that the Log Analytics workspace is created and accessible before linking it with the AKS cluster.
- The retention_days attribute should comply with organizational policies regarding data retention and privacy.
- Default values for attributes should be defined based on best practices or organizational standards, if applicable."
  type = object({
    enabled        = bool
    category       = optional(list(string))
    category_group = optional(list(string))
