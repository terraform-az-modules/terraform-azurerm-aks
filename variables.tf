...
variable "role_based_access_control" {
  ...
  description = "Configures role-based access control (RBAC) settings for the Azure Kubernetes Service (AKS) cluster. * managed: Whether Azure RBAC is managed by Azure AD. * tenant_id: (Optional) The Tenant ID which should be used for authentication. * azure_rbac_enabled: Whether to enable Azure RBAC for Kubernetes authorization."
  }))
  default = null
}
