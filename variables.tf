...
variable "role_based_access_control" {
  ...
  description = "Configures role-based access control (RBAC) settings for the Azure Kubernetes Service (AKS) cluster. * managed: Specifies whether Azure manages the RBAC. * tenant_id: (Optional) The Tenant ID which should be used for authentication. * azure_rbac_enabled: Specifies whether Azure RBAC is enabled fo..."
  }))
  default = null
}
